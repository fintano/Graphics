...
public class Camera {
	...
	private float[] nAxis = new float[3];
	private float fovy;//
	...
	public void BaseYaw(float dYaw){
		//float length = FloatMath.sqrt(pos[0]*pos[0] + pos[2]*pos[2]);
		float rad_angle = TO_RADIAN * dYaw * DELTA_CONSTANT * 10.0f;
		float sina = FloatMath.sin(rad_angle);
		float cosa = FloatMath.cos(rad_angle);
		
		
		pos[0] = pos[0]*cosa - pos[2] * sina ; pos[2] = pos[0]*sina + pos[2]*cosa;
		
		nAxis[0] = pos[0]; nAxis[1] =  pos[1] ; nAxis[2] = pos[2];
	...
	public void Zoom(double d) {
		...
		fovy += d * DELTA_CONSTANT;
	}

	public void nomalize(float[] vec) {
		float amp = FloatMath.sqrt(vec[0] * vec[0] + vec[1] * vec[1] + vec[2] * vec[2]);
		vec[0] /= amp;
		vec[1] /= amp;
		vec[2] /= amp;
	}
